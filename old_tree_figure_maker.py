...
parser.add_argument("--psipred_rgb", 
					"-pr", 
					action="store_true", 
					help="creates a figure containing a tree with a \
					psipred rgb color map attached (r = coil, g = helix\
					, b = strand)")
...
if args.pfam_full:
	...
	domain_color_dict["0"] = "red"
	domain_color_dict["1"] = "lime"
	domain_color_dict["2"] = "blue"
	...
	def pfam_layout(node):
		...
		if node.is_leaf():
			...
			#print build_pfam_motif_face(domain_dict[node.name])
			motif_face = SeqMotifFace(seq, build_pfam_motif_face(domain_dict[node.name]), scale_factor=16, seq_format = "compactseq")
...
if args.psipred_rgb:
	tree = Tree(input_tree, format=1)
	number_of_sequences = len(tree.get_leaf_names())
	print str(number_of_sequences) + " tips in tree file."
	psipred_data = convert_to_dataframe(data_file).split("\n")[1:-1]
	print str(len(psipred_data)) + " entries in data file."
	structure_dict = {} #header:data dictionary constructed below	
	for psipred_prediction in psipred_data:
		structure_dict[psipred_prediction.split("\t")[0]] = psipred_prediction.split("\t")[1:]
	#for key in structure_dict:
	#	print structure_dict[key]
	def make_rgb_profile(ss_data):
		ss_profiles = []
		for index, value in enumerate(ss_data[:-1]):
			if value != "NaN":
				ss_color = QColor()
				ss_color.setRgb(float(255)*float(value.split(",")[6]),
								float(255)*float(value.split(",")[7]),
								float(255)*float(value.split(",")[8])
								)
				print ss_color.name()
				ss_profiles.append([index, 
									index + 1, 
									"[]", 
									0, 
									args.pfam_data_width / number_of_sequences, #CHANGE THIS TO PSIPRED LATER 
									"transparent",#domain_color_dict[data[data_index]], 
									str(ss_color.name()), #hexcode for RGB value 
									None])
		return ss_profiles
		
	def psipred_layout(node):
		...
		if node.is_leaf():
			...
			seq = "A" * len(structure_dict[node.name][:-1])
			#print len(seq)
			#print seq
			nameface = TextFace(node.name)
			add_face_to_node(nameface, node, 0, aligned=True) #display name
			motif_face = SeqMotifFace(seq, make_rgb_profile(structure_dict[node.name]), scale_factor=16, seq_format = "compactseq")
			#motif_face.border.width = 10
			#motif_face.border.color = "pink"
			#motif_face.inner_border.width = 2
			#motif_face.inner_border.color = "orange"
			node.add_face(motif_face, column=1, position = "aligned") #display psipred rgb values
